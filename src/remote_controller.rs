...
use crate::{
    ...
    net::{
        request::{RawDbRequest, RawFileRequest, RemoteRequest, SubscribeArgs, UnsubscribeArgs},
...
pub fn spawn(config: RemoteConfig, c_token: CancellationToken) -> JoinHandle<Result<()>> {
    tokio::spawn(async move {
        ...
        let res = tokio::select! {
            res = run(local_port, tx_db_request, tx_file_request, c_token.clone()) => res,
